...
- Requires `OPENROUTER_API_KEY` stored as a script property. Protect this key; avoid logging or committing it.
...
- Sending email content to OpenRouter (and underlying model providers) may have privacy/regulatory implications; ensure alignment with organizational policies.
...
## Recommendations
1. Document user consent for LLM processing if multiple stakeholders access the mailbox.
2. Rotate `OPENROUTER_API_KEY` periodically and revoke keys on compromise.
3. Consider redacting sensitive sections of emails before sending to OpenRouter if privacy is a concern.
