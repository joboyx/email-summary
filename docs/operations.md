...
- **Logs**: Review Apps Script execution logs via `npm run watch` (stream in terminal) or `npm run watch:open` (browser). Look for `Error in summarizeAndSendDailyEmail` entries or OpenAI call failures.
...
3. Verify status of Gmail quota and OpenAI usage limits.
4. Inspect script properties to ensure `OPENAI_API_KEY` remains valid.
...
- **OpenAI**: Token throughput and rate limits; request is sized for individual emails, so throughput depends on inbox volume.

## Maintenance Tasks
- Rotate `OPENAI_API_KEY` periodically and update script properties accordingly.
...
## Incident Response
- Failed OpenAI responses are logged but skipped; monitor for repeated failures, which may result in missing summaries.
